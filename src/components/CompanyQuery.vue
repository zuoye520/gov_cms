...
//					apps.setSessionStorage('SEARCH_PARAMS',this.queryParams);
					if(this.queryParams.level =='信息身份'||this.queryParams.level =='请选择'){
						window.location.href =`${context}/enterpriseList?fatherCategory=101&category=21&ename=${this.queryParams.ename}&pname=${this.queryParams.pname}&level=${this.queryParams.level}`;
					}else{
